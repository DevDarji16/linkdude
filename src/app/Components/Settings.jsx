  ...
  const { setLoggedInData } = useContext(LoggedIn);
    ...
    return (
        <div className='relative ' onClick={(e) => e.stopPropagation()}>
            <div className={`${settings ? 'opacity-100' : 'opacity-0 pointer-events-none'} w-52   duration-300 transition-all ease-in-out rounded-lg flex flex-col ${theme === 'light' ? 'bg-[#181b1e] text-white' : 'bg-gray-100'} items-center absolute bottom-24 sm:bottom-14 left-8  shadow-lg border p-2`}>
                ...
                <div className={`${theme==='light'?'border-gray-700':'border-gray-300'} border  w-full my-2`}></div>
                <Link href={'/rate-us'} className='w-full'>
                    <div className={`${theme === 'dark' ? 'hover:bg-gray-200' : 'hover:bg-[#24292e]'} text-center text-[13px] rounded-lg p-1`}>Rate us</div>
                </Link>
                <Link href={'/contact-us'} className='w-full'>
                    <div className={`${theme === 'dark' ? 'hover:bg-gray-200' : 'hover:bg-[#24292e]'} text-center text-[13px] rounded-lg p-1`}>Contact us</div>
                </Link>
            ...
            <div className='p-3 absolute bottom-12  sm:bottom-0   '>
