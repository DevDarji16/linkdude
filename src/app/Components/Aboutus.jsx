...
const Aboutus = () => {
  ...
  return (
    ...
    <div className={`${theme === 'dark' ? 'bg-white text-black' : 'bg-black text-white'}  w-screen overflow-y-auto`}>
      ...
      <main className={`h-full w-full  max-w-4xl mx-auto px-4 pt-28 py-12 selection:text-green-500`}>
        ...
        <h1 className="text-4xl font-bold mb-4 text-center font-myfont">About Linkdude</h1>

        <p className={`text-lg mb-8 text-center ${theme === 'dark' ? 'text-gray-700' : 'text-gray-300'}`}>
        Linkdude is a modern <span className='bg-gradient-to-tr from-blue-400 to-purple-700 text-transparent bg-clip-text font-bold'>link-in-bio</span> platform that helps users showcase their content and organize multiple spaces — with more features like web templates coming soon.
  ...
  The frontend of Linkdude is built using <span className="font-medium">Next.js (App Router)</span> and <span className="font-medium">Tailwind CSS</span>, focusing on delivering a clean, responsive, and accessible user experience. The interface is designed with modern UI practices, reusable components, and performance in mind to ensure seamless navigation across all devices.
</p>
          ...
          <h1 className='font-bold text-xl'>Mehul Soni</h1>
            ...
            <p className={`${theme === 'light' ? 'text-gray-300' : 'text-gray-800'} text-center`}>
  The backend of Linkdude is developed using <span className="font-medium">Node.js</span>, <span className="font-medium">Express</span>, and <span className="font-medium">MongoDB</span>. It manages user authentication, database operations, API routing, and all core server-side functionality to ensure secure and efficient data handling.
</p>
        ...
        <div className={`mt-12 text-center ${theme === 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
          <p>We’ve worked together to make Linkdude powerful, customizable, and easy to use for creators everywhere.</p>
        </div>
      </main>
    </div>
      <Footer/>
